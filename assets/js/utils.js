...
KH.utils = (() => {
  ...
  const parseAmount = (str) => Number(String(str || '').replace(/[^0-9]/g, '')) || 0;
  ...
  function openModal(html, opts) {
    ...
    root.innerHTML =
      '<div class="modal-backdrop">' +
        '<div class="modal ' + (opts.sheet ? 'modal-sheet' : '') + '" role="dialog" aria-modal="true">' + html + '</div>' +
      '</div>';
    ...
    return new Promise((resolve) => {
      openModal(
        '<div class="confirm-box">' +
          ...
          '<h3>' + esc(opts.title || 'Konfirmasi') + '</h3>' +
          '<p class="muted">' + esc(opts.message || '') + '</p>' +
          '<div class="row-gap">' +
            '<button class="btn btn-ghost" id="cf-no">' + esc(opts.cancelText || 'Batal') + '</button>' +
            '<button class="btn ' + (opts.danger ? 'btn-danger' : 'btn-primary') + '" id="cf-yes">' + esc(opts.confirmText || 'Ya, lanjutkan') + '</button>' +
          '</div>' +
        '</div>'
      );
  ...
  return {
    fmtIDR, fmtNum, parseAmount, esc, uid, pad, toISO, todayISO, nowTime,
    monthKey, monthLabel, fmtDate, fmtDateLong, daysUntil, debounce,
    toast, openModal, closeModal, sheetHead, wireClose, confirmDialog
  };
})();
